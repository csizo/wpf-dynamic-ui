...
namespace Csizmazia.WpfDynamicUI.WpfDynamicControl.ModelDiscovery
{
    public static class DiscoveryExtensions
    {
        ...
        public static PropertyInfo GetPropertyForSelectedItem(this PropertyInfo propertyInfo, Type viewModelType)
        {
            ...
            var collectionItemType = propertyInfo.PropertyType.GetCollectionItemType();

            return
                viewModelType.GetInstanceProperties()
                    .SingleOrDefault(
                        pi =>
                             pi.PropertyType.IsAssignableFrom(collectionItemType) &&
                            SelectedItemPropertyFilter.GetPossiblePropertyNames(propertyInfo).Contains(pi.Name));
        }
